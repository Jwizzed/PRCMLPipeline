...
def calculate_and_aggregate_features(
    ...
    def isa(alt):
        ...
        if 0 < alt <= trop:
            ...
            pressure = p0 * (temperature / T0) ** (
                        (-1) * 9.80665 / (betabelow * R))
        elif trop < alt < 47000:
            temperature = T0 + betabelow * trop
            pressure = (p0 * (temperature / T0) ** (
                        (-1) * 9.80665 / (betabelow * R))) * math.exp(
                -9.80665 * (alt - trop) / (R * temperature))
    ...
    def calculate_true_airspeed(row):
        ...
        V = np.sqrt((GS * np.sin(track) - u_wind) ** 2 + (
                    GS * np.cos(track) - v_wind) ** 2) * ktstofts
        return V
    ...
    def calculate_wind_acceleration(group):
        group = group.sort_values('timestamp')
        group['W_long'] = (group['u_component_of_wind'] * np.sin(
            np.radians(group['track'])) +
                           group['v_component_of_wind'] * np.cos(
                    np.radians(group['track']))) * ktstofts
        group['dWi_dt'] = group['W_long'].diff() / group[
            'timestamp'].diff().dt.total_seconds()
        return group
    ...
    def identify_flight_phases(group, flight_phases_refinement=False):
        ...
        if flight_phases_refinement:
            ...
            initial_climb_phase = initial_climb_phase[
                (initial_climb_phase['ROC'] > initial_climb_phase[
                    'ROC'].quantile(0.25)) &
    ...
    def calculate_thrust_minus_drag_for_phases(trajectory_df,
        ...
        for flight_id, group in tqdm.tqdm(trajectory_df.groupby('flight_id'),
                                          desc="Calculating T-D for each flight"):
            ...
            relevant_phase['V'] = relevant_phase.apply(calculate_true_airspeed,
                                                       axis=1)
            ...
            relevant_phase['T_minus_D'] = (
                    32.17405 * relevant_phase['dh_dt'] / relevant_phase[
                'V'] * (relevant_phase['temperature'] / (
    ...
    if use_trajectory:
        ...
        merged_df = pd.merge(train_df, aggregated_features,
