...
from kubeflow.components.data_preprocessing.select_features import \
    feature_selection, drop_features, process_category_split
...
def flight_tow_pipeline(data_path: str, external_data_path: str):
    ...
    exclude_cols = ['MTOW(kg)', 'flight_id', 'tow', 'date', 'callsign', 'adep',
                    'ades', 'actual_offblock_time', 'arrival_time',
                    'aircraft_type',
                    'wtc', 'airline']
    ...
    split_mtow_task = split_by_mtow(
        input_file=initial_drop_task.outputs["output_file"]
    )
    ...
    for category in categories:
        category_drop_task = drop_features(
            input_file=split_mtow_task.outputs[f"X_{category}_output"],
            final_drop=True
        )
        ...
        ensemble_task = train_ensemble_model(
            x_train_file=category_feature_selection.outputs[
                "X_train_selected_file"],
            y_train_file=category_split_task.outputs["y_train_output"],
            x_test_file=category_feature_selection.outputs[
