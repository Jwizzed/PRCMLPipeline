...
@component(packages_to_install=["pandas", "scikit-learn", "xgboost", "catboost"])
def train_ensemble_model(
    ...
    results_df = pd.DataFrame({"flight_id": X_train["flight_id"], "tow": ensemble_pred})
    results_df.to_csv(predictions_file, index=False)

    metrics_df = pd.DataFrame(
        {
            "metric": ["CatBoost RMSE", "XGBoost RMSE", "Ensemble RMSE"],
            "value": [catboost_rmse, xgboost_rmse, ensemble_rmse],
        }
    )
