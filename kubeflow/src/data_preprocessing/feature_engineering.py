...
class FeatureEngineering:
    ...
    def calculate_true_airspeed(self, row):
        ...
        V = np.sqrt((GS * np.sin(track) - u_wind) ** 2 + (
                    GS * np.cos(track) - v_wind) ** 2) * self.ktstofts
        return V
    ...
    def calculate_wind_acceleration(self, group):
        ...
        group["W_long"] = (group["u_component_of_wind"] * np.sin(
            np.radians(group["track"])) +
                    ...
                    np.radians(group["track"]))) * self.ktstofts
    ...
    def process_data(self, trajectory_df, train_df, use_trajectory,
                     flight_phases_refinement):
        if use_trajectory:
            td_df = self.calculate_thrust_minus_drag_for_phases(trajectory_df,
                                                                flight_phases_refinement)
