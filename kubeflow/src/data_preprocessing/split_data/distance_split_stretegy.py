...
class DistanceSplitStrategy(SplitDataStrategy):
    ...
    def split(self, df: pd.DataFrame, **kwargs) -> Dict[
        ...
        wide_body_df['flown_distance_range'] = wide_body_df[
            'aircraft_type'].apply(
            lambda x: 'B77W' if x == 'B77W' else 'NonB77W'
        )

        result = {'0-500': self.process_subset(
            narrow_body_df[narrow_body_df['flown_distance_range'] == '0-500']),
            '500-1000': self.process_subset(
                narrow_body_df[
                    narrow_body_df['flown_distance_range'] == '500-1000']),
            'above_1000': self.process_subset(
                narrow_body_df[
                    narrow_body_df['flown_distance_range'] == '>1000']),
            'non_b77w': self.process_subset(
                wide_body_df[
                    wide_body_df['flown_distance_range'] == 'NonB77W']),
