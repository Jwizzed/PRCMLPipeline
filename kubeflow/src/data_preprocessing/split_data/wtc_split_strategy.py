...
class WTCSplitStrategy(SplitDataStrategy):
    ...
    def split(
        ...
        X_wtc_M = (
            X[X["wtc_M"] == 1].drop(["wtc_H", "wtc_M"], axis=1).reset_index(drop=True)
        )
        X_wtc_H = (
            X[X["wtc_H"] == 1].drop(["wtc_H", "wtc_M"], axis=1).reset_index(drop=True)
        )
        ...
        return {"wtc_M": (X_wtc_M, y_wtc_M), "wtc_H": (X_wtc_H, y_wtc_H)}
