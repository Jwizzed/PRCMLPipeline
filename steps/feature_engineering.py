...
def feature_engineering(
    ...
    if config.USE_EXTERNAL_DATA:
        ...
        train_df = pd.merge(train_df, external_df, on="aircraft_type",
                            how="left")
        test_df = pd.merge(test_df, external_df, on="aircraft_type",
                           how="left")

        aircraft_types_with_info = external_information.keys()
        train_df = train_df[
            train_df["aircraft_type"].isin(aircraft_types_with_info)]
    ...
    exclude_cols = [
        "flight_id", "tow", "date", "callsign", "adep", "ades",
